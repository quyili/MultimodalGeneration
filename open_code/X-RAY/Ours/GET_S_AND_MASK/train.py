...
tf.flags.DEFINE_string('M', './DATA/chest_xray/train/M', 'X files for training')
tf.flags.DEFINE_string('F', './DATA/chest_xray/train/NEW_F2_', 'X files for training')
tf.flags.DEFINE_string('M_test', './DATA/chest_xray/test/M', 'X files for training')
tf.flags.DEFINE_string('F_test', './DATA/chest_xray/test/F2_', 'X files for training')
tf.flags.DEFINE_string('load_model', '20200223-0538',
...
def train():
    with tf.device("/cpu:0"):
        ...
        with graph.as_default():
            ...
            FG_ave_grad = average_gradients(G_grad_list)
            D_ave_grad = average_gradients(D_grad_list)
            FG_optimizer_op = G_optimizer.apply_gradients(FG_ave_grad)
            D_optimizer_op = D_optimizer.apply_gradients(D_ave_grad)
            optimizers = [FG_optimizer_op, D_optimizer_op]
