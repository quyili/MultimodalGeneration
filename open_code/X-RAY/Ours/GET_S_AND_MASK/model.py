...
from GAN_feature_discriminator import FeatureDiscriminator
from GAN_encoder import GEncoder
from GAN_decoder import GDecoder
...
class GAN:
    def __init__(self,
        ...
        self.EC_F = GEncoder('EC_F', ngf=ngf,units=units,keep_prob=self.keep_prob[0])
        self.DC_F = GDecoder('DC_F', ngf=ngf, output_channl=2,units=units)

        # self.G_M = Unet('G_M', ngf=ngf/2, keep_prob=0.9)
    ...
    def model(self, f):
        ...
        code_f_mean, code_f_logvar = self.EC_F(f*0.95+0.05)
        ...
        D_loss += self.mse_loss(j_code_f_rm, 1.0) * 2.5
        D_loss += self.mse_loss(j_code_f, 0.0)* 2.5
        FG_loss += self.mse_loss(j_code_f, 1.0) * 0.001

        FG_loss += self.mse_loss(tf.reduce_mean(code_f_mean), 0.0) * 0.00001
        FG_loss += self.mse_loss(tf.reduce_mean(code_f_std), 1.0) * 0.00001

        # 使得随机正态分布矩阵解码出结构特征图更逼真的对抗性损失
        D_loss += self.mse_loss(j_f, 1.0) * 2.5
        D_loss += self.mse_loss(j_f_rm, 0.0) * 2.5
        FG_loss += self.mse_loss(j_f_rm, 1.0) * 0.01

        # 结构特征图两次重建融合后与原始结构特征图的两两自监督一致性损失
        FG_loss += self.mse_loss(f_one_hot, f_r_prob) * 80

        FG_loss += tf.reduce_mean(tf.abs(f_one_hot-f_r_prob)) * 1
    ...
    def image_summary(self, image_list):
        f, f_r, f_rm, f_one_hot, f_r_prob= image_list[0], image_list[1], image_list[2], image_list[3], image_list[4]
