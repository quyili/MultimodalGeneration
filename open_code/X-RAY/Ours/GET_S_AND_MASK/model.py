...
class VAE_GAN:
    ...
    def model(self, s, m):
        ...
        code_f_g = tf.reshape(code_f_g, shape=[self.input_shape[0], 64, 64, -1])
        ...
        GS_loss += (tf.reduce_mean(s_r_prob[:, :, :, 0]) - tf.reduce_mean(s_r_prob[:, :, :, 1])) * 0.0001
        GS_loss += (tf.reduce_mean(s_g_prob[:, :, :, 0]) - tf.reduce_mean(s_g_prob[:, :, :, 1])) * 0.0001
