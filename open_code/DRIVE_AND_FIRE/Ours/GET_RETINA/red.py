...
class Encoder:
    ...
    def __call__(self,in_image):
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            # deconv 7
            deconv7 = tcl.conv2d_transpose(deconv6, self.filter_size, self.kernel_size, self.conv_stride, padding='valid',
                                           weights_initializer=tf.random_normal_initializer(stddev=self.initial_std), \
                                           biases_initializer=tf.zeros_initializer(), activation_fn=None)
            deconv7 = tf.nn.relu(deconv7)
            # deconv 8 + shortcut
            deconv8 = tcl.conv2d_transpose(deconv7, self.filter_size, self.kernel_size, self.conv_stride, padding='valid',
                                           weights_initializer=tf.random_normal_initializer(stddev=self.initial_std), \
                                           biases_initializer=tf.zeros_initializer(), activation_fn=None)
            deconv8 += shortcut_deconv8
            deconv8 = tf.nn.relu(deconv8)
            # deconv 9
            deconv9 = tcl.conv2d_transpose(deconv8, self.filter_size, self.kernel_size, self.conv_stride, padding='valid',
                                           weights_initializer=tf.random_normal_initializer(stddev=self.initial_std), \
                                           biases_initializer=tf.zeros_initializer(), activation_fn=None)
