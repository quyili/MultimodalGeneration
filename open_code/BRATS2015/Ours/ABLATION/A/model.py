...
class GAN:
    ...
    def model(self,  x, y, z, w):
        ...
        x_g = self.G_X(tf.random_normal(self.input_shape, mean=0., stddev=1., dtype=tf.float32), cx_code)
        y_g = self.G_X(tf.random_normal(self.input_shape, mean=0., stddev=1., dtype=tf.float32), cy_code)
        z_g = self.G_X(tf.random_normal(self.input_shape, mean=0., stddev=1., dtype=tf.float32), cz_code)
        w_g = self.G_X(tf.random_normal(self.input_shape, mean=0., stddev=1., dtype=tf.float32), cw_code)
