...
class SDecoder:
    ...
    def __call__(self, DC_input):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            with tf.variable_scope("add2_conv", reuse=self.reuse):
                add2_conv = tf.layers.conv2d(inputs=add2, filters=2 * self.ngf, kernel_size=3, strides=1,
                                             padding="SAME",
                                             activation=None,
                                             kernel_initializer=tf.random_normal_initializer(
                                             ...
                                             bias_initializer=tf.constant_initializer(0.0),
                                             name='add2_conv')
                add2_norm = ops._norm(add2_conv, self.is_training, self.norm)
                output = ops.relu(add2_norm)
