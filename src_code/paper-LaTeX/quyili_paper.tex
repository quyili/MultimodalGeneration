...
To solve these problems, this study proposed a multimodal medical image synthesis method based on Generative Adversarial Networks, which can synthesize registration multimodal medical images with specified lesions. Experiments on multiple datasets have comprehensively verified the effectiveness of synthetic lesions and the availability of synthetic images. The main contributions is are as follows:
...
\end{figure*}
\subsection{本文的创新点}
针对本研究面临的各项挑战，本文提出了一种基于GAN的多模态医学影像合成方案，可从随机矩阵合成具有指定病灶的配准多模态医学影像，并在多个数据集上充分验证了合成方法在多种不同成像模态、成像部位和病灶种类的医学影像合成中鲁棒性、合成医学影像的病灶有效性、合成多模态影像的配准有效性和合成影像在多种病灶处理任务中的可用性。合成的医学影像可用于智能医学影像处理任务的预训练和数据增强并能在任务中显著提升模型的泛化能力，以此可有效缓解智能医学影像处理任务中数据不足的问题。本文的创新点包括：

\begin{itemize}
	\item 本文提出了一种基于Sobel算子（Sobel Operator）\cite{147Sobel}和变分自动编码器的结构特征图提取和合成方法。提取结构特征图时，无需额外的结构信息标签和标签提取训练，可直接从真实影像提取出结构特征图。相较于当前最好的结构信息提取方法，本文方法提取的结构特征图在更具核心结构描绘能力的同时更加干净简洁、线条清晰、完整合理。生成结构特征图时，本文方法可直接从随机正态分布矩阵合成任意数量的、具有充分多样性的结构特征图。
...
\begin{figure}[thbp]
	...
	\label{image_and_f}
\end{figure}
不同模态的医学影像从成像原理到视觉呈现都有诸多差异，一项通用的医学影像合成方案应能在常见的多种模态中表现良好才具有较好的鲁棒性。在带病灶的医学影像合成中，对病灶的评估也应考虑到常见的多种智能医学影像病灶处理任务。因此，本研究在六个公开数据集中对提出的方案进行了充分的实验验证，实验涵盖了RGB彩色照片、X-ray、MRI、CT等常见的医学影像成像模态，涵盖了脑部、视网膜、肺部截面、胸部等多个病变部位，涵盖了肿瘤分割、血管分割、肺炎分类、肺结节检测等多种智能医学影像病灶处理任务。这六个公开数据集的详细情况和数据预处理方式如下：
\begin{itemize}
...
\begin{table}[thbp!]
	...
	\begin{center}
		...
		\begin{tabular}{cllll}
			...
			训练数据 &测试数据 & mAP↑   \\
			\hline
			\tabincell{c}{TC Lung CT训练集}  	 	&TC Lung CT测试集 	&0.443 
			\\
			\tabincell{c}{TC Lung CT训练集+20000张\\合成图像}	&TC Lung CT测试集  	&\textbf{0.457} \\	
			\hline
		\end{tabular}
	\end{center}
\end{table}
...
最后，本研究在多种医学影像数据集和多种病灶处理任务中验证了对合成医学影像用作智能医学影像处理任务的预训练数据或增强数据的可用性、其中合成病灶的有效性和合成多模态的配准程度。多项实验的结果表明，本研究的合成方法在多种不同成像模态、成像部位和病灶种类的医学影像中表现鲁棒，合成的医学影像数据可以用作多种智能医学影像处理任务的预训练数据或增强数据，并能显著提高模型的泛化能力。
