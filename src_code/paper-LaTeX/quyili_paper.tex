...
The main contributions is are as follows:
...
3.In this study, several general indicators were used to directly evaluate the quality of the synthesized images. Meanwhile, the performance of the synthesized images was indirectly evaluated by applying the synthesized images to train the intelligent medical image processing models and evaluate the trained models. The results showed that the synthesized image can be used for pre-training and data augmentation in a variety of tasks to improve model performance.
\\
...
此外，对于医学影像，其最有价值的地方在于其中包含的病灶（Lesion）信息。医学影像中的病灶信息是医生进行诊断的重要依据，也是智能医学影像处理模型推理诊断的重要依据。然而，当前绝大多数的医学影像合成的研究中，未对病灶信息的合成进行任何针对性研究，致使合成的医学影像尽管整体上与真实影像相似但关键的病灶信息却很难确保有效地合成，即使影像中合成了病灶也难以提供对应的病灶标签，难以对合成的医学影像在医疗诊断上的可用性进行评估和检验，更很难单独地对合成病灶的有效性进行评估和检验。这同样是合成医学影像研究中未能很好解决的一个问题。
...
总的来说，当前合成医学影像的研究中还存在合成模态数量少、难以合成配准多模态、合成病灶不可控、合成数量有限制、合成生理结构不合理、合成质量评价不客观等各项未能很好解决的问题。本研究的目的正是针对上述的这些问题，基于当前的图像合成技术和相关研究成果，进一步出探索一套完整的可以同时解决上述多个问题的解决方案，实现可控制病灶合成并带有病灶标签的配准的多模态医学影像的合成。
...
本文在前两个章节中详细介绍了研究的技术路线和相关的研究现状，在本章节中，将对智能医学影像处理研究中的一些研究基础进行介绍，本文后续章节中提出的方法和实验评估将对本章介绍的基础方法和模型进行调用、改进和扩展。本章将基于卷积神经网络的图像分类、基于生成对抗网络的图像合成、基于变分自编码器的图像合成、基于卷积神经网络的语义分割和基于卷积神经网络的目标检测进行依次介绍。
...
最后，本研究在多种医学影像数据集和多种病灶处理任务中验证了对合成医学影像用作智能医学影像处理任务的预训练数据或增强数据的可用性、合成影像中病灶的有效性和合成多模态之间的配准程度。多项实验的结果表明，本研究的合成方法在多种不同成像模态、成像部位和病灶种类的医学影像中表现鲁棒，合成的医学影像数据可以用作多种智能医学影像处理任务的预训练数据或增强数据，并能显著提高模型的泛化能力。
...
\noindent
[2]. Yili Qu, Chufu Deng, Wanqi Su, Ying Wang, Yutong Lu and Zhiguang Chen, Nong Xiao. Multimodal Brain MRI Translation Focused on Lesions[C]//2020 12th International Conference on Machine Learning and Computing (ICMLC). ACM, 2020. (第一作者, EI)
