...
\begin{CJK*}{GBK}{song}
	...
	图\ref{ablation}为消融实验量化评估结果的柱状图对比情况，不难看出，本文合成方案中的每一项方法都起到了针对性的提升效果，其中，采用结构特征图作为输入对评估结果的提升最为突出，其次是对结构特征图融合随机噪声和在对病灶标签过滤之后添加病灶生成指导损失产生的提升。图中的数据中值得注意的是，不对输入的病灶标签进行过滤，合成的影像中不止会存在病灶结构不合理的影像，与真实数据集进行相似度评估的结果也会因此难以提升甚至下降。
	\begin{figure}[thbp!]
		\centering
	...
	从实验结果看，同在真实数据上训练的第\ref{registration}节中的模态转换方法与当前最好的模态转换方法pGAN\cite{69dar2018image}效果相当，这表明本研究的模态转换方法是比较优秀的。然后，再在10倍真实数据量的合成多模态配准脑MRI数据上训练了本研究方法中的模态转换器，结果表明，训练出的模型在测试集上的效果已经较为接近在真实数据上训练的模型，这意味着合成多模态影像不仅整体质量与真实影像较为接近，也意味着合成的多模态影像之间具有较高的配准度。在进一步对真实数据和合成数据随机混合的训练中，合成数据起到数据增强的效果，大大提升了模型的转换效果，取得了比当前最好的转换方法更好的转换结果。由此表明，本研究的模态配准方案是效果良好的，合成的多模态影像具有较高的配准度，能在对配准度有较高要求的医学影像的任务中作为增强数据使用。
	
	\begin{figure}[thbp!]
		\centering
		\includegraphics[width=0.85\linewidth]{figures/trans_res}
...
%\noindent
%[4]. Wanqi Su, Yili Qu, Chufu Deng, Ying Wang, Fudan Zheng, Zhiguang Chen. Enhance Generative Adversarial Network By Discrete Wavelet Transform to Denoise Low-Dose CT Images[C]//27th IEEE International Conference on Image Processing (ICIP). IEEE, 2020. (第二作者, 审稿中, CCF C)
%
%\noindent
...
\noindent
[1]. 基于超级计算机的大数据处理支撑平台研究. 国家自然科学基金委员会项目. 项目批准号: 61872392. 负责人: ***. 依托单位: 中山大学. (项目组主要成员)

\noindent
[2]. 基于生成对抗网络的多模态医学影像合成.中山大学优秀研究生创新发展项目.批准号:19lgyjs61.依托项目:面向典型应用的虚拟数据空间验证与优化,依托项目类别:国家重点研发计划课题,依托项目负责人:***.项目负责人:***.承担单位:中山大学.(项目组主要成员)
