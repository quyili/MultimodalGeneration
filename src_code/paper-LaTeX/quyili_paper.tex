...
\begin{figure}[thbp!]
	...
	\caption{各数据集中医学图像和提取的结构特征图。（a）为DRIVE视网膜图像数据集。（b）为Kaggle Chest X-Ray数据集。（c）Kaggle Lung CT数据集。（d）为天池全球数据智能大赛(2019)数据集。以上子图中从左到右依次为原图、结构特征图、掩膜、融合了随机噪声结构特征图。（e）为BRATS2015数据集，其中依次为T1、T2、T1c、Flair四个模态的MRI、从T1提取出来的结构特征图、去掉肿瘤病灶结构信息的结构特征图。}
	\label{image_and_f}
\end{figure}
...
\subsection{训练设置}
如无特殊说明，每个实验的迭代次数等于训练数据集的100个epoch。 学习率为1e-4，无权重衰减，使用beta为1为0.5的Adam优化器，批处理大小为1。 评估结果是2D图像上多模态结果的平均值，每个实验都经过了4次训练保留最佳结果。
...
\begin{table*}[thbp!]
	...
	\begin{tabular}{c|c|c|c|c|c}
		%\toprule
		\hline
		实验编号		&\tabincell{c}{输入结构\\特征图} &\tabincell{c}{掩膜过滤\\病灶标签} &\tabincell{c}{输入融合\\随机噪声} &\tabincell{c}{模态配\\准损失$\mathcal{L}_{reg}$} &\tabincell{c}{病灶生成\\指导损失$\mathcal{L}_{les}$}  \\
		%\midrule
		\hline
...
\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/ablation}
	\caption{消融实验量化评估结果的柱状图。}
	\label{ablation}
\end{figure}
\subsection{合成方法在不同数据集上的量化评估}
如表~\ref{evalu_on_all_dataset1}和表~\ref{evalu_on_all_dataset2}所示，我们的方法在各个数据集上进行了量化评估，并与当前最先进的结果进行对比。
表~\ref{evalu_on_all_dataset1}中，我们与\cite{96zhang2019skrgan:}中的数据进行了对比，数据表明，在Chest X-ray和Lung CT两个数据集上，我们的整体合成质量优于表中其他方法的。表~\ref{evalu_on_all_dataset2}中，由于\cite{96zhang2019skrgan:}中两份数据集的不可获取，我们在Color Fundus和BRATS MRI两个与原文类似数据集上，对各个方法进行了复现，复现结果与表~\ref{evalu_on_all_dataset1}中两个数据集上的结果是一致的。表~\ref{evalu_on_all_dataset2}中，在BRATS MRI数据集上，我们的方案可直接进行多模态合成，其余方案需对每个模态单独训练后合成，最后的评估结果是多个模态上评估结果的平均值。此外，表~\ref{evalu_on_all_dataset2}中，在BRATS MRI数据集上，我们添加了肿瘤分割标签作为病灶生成指导标签，评估时，我们根据病灶标签对合成数据集中影像与原数据集中影像的肿瘤切割出来进行评估，并与完整影像的评估进行比对，结果表明，我们的合成肿瘤信息与真实肿瘤具有极高的相似度，并在整体影像的评估中比整体平均情况更好，这说明我们的病灶生成指导方法是有效的，这对于合成影像的应用也具有重大意义。
\begin{table}[thbp!]
	\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
	\begin{center}
	...
	\begin{center}
		...
		\resizebox{\textwidth}{18mm}{
			\begin{tabular}{ll|lllllll}
				...
				%			\multicolumn{2}{c|}{Evaluation}&\multicolumn{6}{c}{Method}\\
				%			\hline
				Dataset &Metric &Ours$^*$ &Ours &SkrGAN$^+$\cite{96zhang2019skrgan:,51ronneberger2015u-net:} &DCGAN\cite{97radford2015unsupervised} &ACGAN\cite{98odena2016conditional} &WGAN\cite{99arjovsky2017wasserstein} &PGGAN\cite{100karras2017progressive}\\
				\hline
				\multirow{2}*{\tabincell{l}{\textbf{Color}\\\textbf{Fundus}}}
%		  		&SWD ↓ &xxx &0.025 &0.160 &0.149 &0.078 &0.036 \\
				&MS-SSIM ↑  &- &0.634 &0.592 &0.389 &0.412 &0.505 &0.496 \\
				&FID ↓  &- &25.13 &35.81 &97.42 &120.2 &247.5 &134.6\\
				\hline
				\multirow{2}*{\tabincell{l}{\textbf{BRATS}\\\textbf{MRI}}}
%				&SWD ↓ &xxx &0.025 &0.160 &0.149 &0.078 &0.036 \\
				&MS-SSIM ↑  &0.638 &0.623 &0.578 &0.331 &0.427 &0.513 &0.489 \\
				&FID ↓  &23.15 &24.27 &30.46 &87.53 &113.8 &197.4 &128.2\\
				\hline
				\\[-6pt]
			\end{tabular}
		}
	...
	以SkrGAN采用的U-net为生成模型、以我们的鉴别器为鉴别器合成多模态影像，以此近似为对SkrGAN的复现。
	\end{center}
\end{table}
...
我们以U-net为分割模型，使用DRIVE视网膜图像训练集+合成视网膜图像及其粗标签数据集，在DRIVE视网膜图像测试集上，采用随机混合模式训练视网膜血管注释模型，与只使用DRIVE视网膜图像训练集的分割结果对比，来验证合成眼底视网膜数据在视网膜血管注释任务中的可用性。从结果看，与SkrGAN合成图像的增强效果对比，我们的合成图像对分割精度的提升更高。实验结果如下表。
\begin{table}[thbp!]
	\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
	\begin{center}
		{\caption{视网膜 synthetic data availability verification experiments.}\label{DRIVE_availability_test}}
		\begin{tabular}{cllll}
			\hline
			\rule{0pt}{12pt}
		   ...
		   \tabincell{c}{DRIVE训练集+2000张\\SkrGAN合成图像}	 &  DRIVE测试集 	& 0.8464 &0.9513 &0.9762 \\
			\tabincell{c}{DRIVE训练集+2000张\\我们的合成图像}	&  DRIVE测试集  	& 0.8736 &0.9627 &0.9811 \\	
			\hline
		\end{tabular}
	\end{center}
\end{table}
\subsubsection{合成胸部X光线数据用于肺炎分类任务}
我们采用Kaggle Chest X-Ray数据集合成了大量带有病变类别标签的胸部X-Ray数据。合成时，由于只有单模态数据，因此无模态配准损失，输入的病灶标签为扩展成的与结构特征图同尺寸独热矩阵的肺炎类别标签。然后再使用Kaggle Chest X-Ray数据集+合成X-Ray数据采用随机混合模式训练了一个胸部X-Ray肺炎分类模型，我们将该模型和只采用Kaggle Chest X-Ray数据集训练的模型进行对比，结果如下表。
\begin{table}[thbp!]
	\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
	\begin{center}
		{\caption{胸部X光线 synthetic data availability verification experiments.}\label{X-Ray_availability_test}}
		\begin{tabular}{cllll}
			\hline
			\rule{0pt}{12pt}
			...
			\\
			\tabincell{c}{X-Ray训练集+2000张\\我们的合成图像}	&  X-Ray测试集  	& 0.842 \\	
			\hline
		\end{tabular}
	\end{center}
\end{table}
\subsubsection{合成肺部CT数据用于肺结节检测任务}
我们采用天池全球数据智能大赛(2019)数据集合成了大量带有病灶检测标签的肺部低剂量CT数据。合成时，由于只有单模态数据，因此无模态配准损失，输入的病灶标签为与结构特征图同尺寸的病灶检测标签。我们使用天池全球数据智能大赛(2019)数据集+合成肺部低剂量CT数据采用随机混合模式训练了一个病灶检测模型，将该模型的测试结果与只采用天池全球数据智能大赛(2019)数据集训练的检测模型进行对比。结果如下表。
\begin{table}[thbp!]
	\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
	\begin{center}
		{\caption{肺部CT synthetic data availability verification experiments.}\label{TC_availability_test}}
		\begin{tabular}{cllll}
			\hline
			\rule{0pt}{12pt}
			...
			\\
			\tabincell{c}{Lung CT训练集+20000张\\我们的合成图像}	&Lung CT测试集  	& 0.737 \\	
			\hline
		\end{tabular}
	\end{center}
\end{table}
...
\begin{figure}[thbp!]
	...
	\caption{Synthetic structural feature maps from random sampling on BRATS2015.}
...
\begin{figure}[thbp!]
	...
	\caption{Synthetic structural feature maps from continuous sampling on BRATS2015.}
	\label{generated_f_continuous_sampling}
\end{figure}
我们对从正态分布进行顺序采样，然后从采样结果解码，获得了如图~\ref{generated_f_continuous_sampling}所示的相邻渐变的结构特征图分布。

\subsubsection{合成医学影像效果展示}
\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/F_to_MRI}
	\caption{Multimodal MRIs systhesized from structural feature maps and lesion labels on BRATS2015.}
	\label{generated_mri}
\end{figure}
...
\begin{figure}[thbp!]
	...
	\caption{X-ray systhesized from structural feature maps.(a) PGGAN\cite{100karras2017progressive}的合成图像，(b) WGAN\cite{99arjovsky2017wasserstein}的合成图像， (c) DCGAN\cite{97radford2015unsupervised}的合成图像，(d) ACGAN\cite{98odena2016conditional}的合成图像，(e) SkrGAN\cite{96zhang2019skrgan:}的合成图像和合成的结构特征图，(f) 我们的方案的合成图像和合成的结构特征图}
	\label{generated_xray}
\end{figure}
图~\ref{generated_xray}中展示了我们在X-ray数据集上合成的结构特征图和X-ray影像，和其他方法合成的结果对比，我们的合成X-ray更加逼真，肋骨和脊柱等细节更加丰富和符合生理逻辑。

\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/SWM_images}
	\caption{视网膜 systhesized from structural feature maps.(a) PGGAN\cite{100karras2017progressive}的合成图像，(b) WGAN\cite{99arjovsky2017wasserstein}的合成图像， (c) DCGAN\cite{97radford2015unsupervised}的合成图像，(d) ACGAN\cite{98odena2016conditional}的合成图像，(e) SkrGAN\cite{96zhang2019skrgan:}的合成图像和合成的结构特征图，(f) 我们的方案的合成图像和合成的结构特征图}
	\label{generated_swm}
\end{figure}
图~\ref{generated_swm}中展示了我们在眼底视网膜数据集上合成的结构特征图和视网膜影像，和其他方法合成的视网膜图像对比，我们采用更少的训练样本合成的视网膜更加逼真，在血管和神经交汇点等细节上更加合理和真实。

\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/FBCT_images}
	\caption{肺部CT systhesized from structural feature maps.(a) PGGAN\cite{100karras2017progressive}的合成图像，(b) WGAN\cite{99arjovsky2017wasserstein}的合成图像， (c) DCGAN\cite{97radford2015unsupervised}的合成图像，(d) ACGAN\cite{98odena2016conditional}的合成图像，(e) SkrGAN\cite{96zhang2019skrgan:}的合成图像和合成的结构特征图，(f) 我们的方案的合成图像和合成的结构特征图}
	\label{generated_FBCT}
\end{figure}
图~\ref{generated_FBCT}中展示了我们在肺部CT数据集上合成的结构特征图和CT影像，和其他方法合成的结果对比，我们的合成CT更加清晰真实。

\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/MRI_images}
	\caption{脑部MRI systhesized from structural feature maps.(a) PGGAN\cite{100karras2017progressive}的合成图像，(b) WGAN\cite{99arjovsky2017wasserstein}的合成图像， (c) DCGAN\cite{97radford2015unsupervised}的合成图像，(d) ACGAN\cite{98odena2016conditional}的合成图像，(e) SkrGAN\cite{96zhang2019skrgan:}的合成图像和合成的结构特征图，(f) 我们的方案的合成图像和合成的结构特征图}
	\label{generated_MRI}
\end{figure}
图~\ref{generated_MRI}中展示了我们和其他方法合成的脑部MRI的对比，可以看出我们的合成的脑MRI更加清晰真实、干净整洁。

\begin{figure}[thbp!]
	\centering
	\includegraphics[width=0.98\linewidth]{figures/MRI_images}
	\caption{TC Lung CT systhesized from structural feature maps.}
	\label{generated_TC}
\end{figure}
图~\ref{generated_TC}中展示了我们在天池肺部CT数据集上合成的结构特征图和CT影像，可以看出我们的合成影像难辨真假，非常逼真。
