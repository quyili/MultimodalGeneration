...
\subsection{实验设置}
\subsubsection{训练设置}
本研究实验采用的硬件环境为一个NVIDIA V100GPU集群，软件环境为Python3.6、CUDA8.0和TensorFlow1.13.2。如无特殊说明，每个实验的迭代次数等于训练数据集的500个epoch。 学习率为1e-5，无权重衰减，使用beta为1为0.5的Adam优化器，批处理大小为1。 评估结果是2D图像上所有模态结果的平均值，每个实验的结果都为经过了至少4次训练保留的最佳结果。
