...
with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    PATH="E:/project/MultimodalGeneration/data/chest_xray/test/NORMAL_X"
    SAVE_F="E:/project/MultimodalGeneration/data/chest_xray/test/NORMAL_F"
    SAVE_M = "E:/project/MultimodalGeneration/data/chest_xray/test/NORMAL_M"
    try:
        os.makedirs(SAVE_F)
        os.makedirs(SAVE_M)
    except os.error:
    ...
    for file in files:
        ...
        fx_ =np.asarray(fx_)[0, :, :, 0, ]
        mask_x_ =np.asarray(mask_x_)[0, :, :, 0, ]
        fx_ = signal.medfilt2d(fx_, kernel_size=3)
        mask_x_ = signal.medfilt2d(mask_x_, kernel_size=17)
