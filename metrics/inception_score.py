...
MODEL_DIR = "D:/project/MultimodalGeneration/metrics"
...
def _init_inception():
    ...
    filepath = os.path.join(MODEL_DIR, "classify_image_graph_def.pb")
...
def inception_score(data_path, model_path):
    globals()['MODEL_DIR'] = model_path
    sess = tf.get_default_session()
    _init_inception()
    return handle(data_path, sess)


def handle(data_path, sess):
    path = pathlib.Path(data_path)
    files = list(path.glob('*.jpg')) + list(path.glob('*.png')) + list(path.glob('*.tif')) + list(
        path.glob('*.tiff')) + list(path.glob('*.mha'))
    x = [np.array(imageio.imread(str(fn))) for fn in files]
