        ...
        with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            try:
                if FLAGS.stage == 'train':
                    ...
                    LD_train_files = read_filename(FLAGS.LD)
                    ...
                    while not coord.should_stop() and epoch <= FLAGS.epoch:
                        ...
                        for b in range(FLAGS.batch_size):
                            ...
                            train_LD_arr = read_file(FLAGS.LD, LD_train_files, index)
                            train_HD_arr = read_file(FLAGS.HD, LD_train_files, index)
                            ...
                            train_true_LD.append(train_LD_arr)
                            train_true_HD.append(train_HD_arr)
                        ...
                        _, train_image_summary_op, train_losses, train_detect_losses,train_bec, train_ssim, train_psnr = sess.run(
                            ...
                            feed_dict={
                                ...
                                HD_0: np.asarray(train_true_HD)[0:1 * int(FLAGS.batch_size / 4), :, :, :],
                        ...
                        if step == 0 or step % int(FLAGS.epoch_steps / 2 - 1) == 0 or step == int(
                            ...
                            val_psnr_list = []
                            val_bec_list = []
                            LD_val_files = read_filename(FLAGS.LD_test)  # 打乱文件名
                            for j in range(int(math.ceil(len(LD_val_files) / FLAGS.batch_size))):
                                val_true_LD = []
                                val_true_HD = []
                                for b in range(FLAGS.batch_size):
                                    val_LD_arr = read_file(FLAGS.LD_test, LD_val_files, val_index)
                                    val_HD_arr = read_file(FLAGS.HD_test, LD_val_files, val_index)
                                    val_true_LD.append(val_LD_arr)
                                    val_true_HD.append(val_HD_arr)
                                    val_index += 1
                                ...
                                val_image_summary_op, val_image_list_0, val_image_list_1, val_image_list_2, val_image_list_3 = sess.run(
                                    ...
                                    feed_dict={
                                        ...
                                        LD_1: np.asarray(val_true_LD)[
                                              1 * int(FLAGS.batch_size / 4):2 * int(FLAGS.batch_size / 4), :, :, :],
                                        HD_1: np.asarray(val_true_HD)[
                                              1 * int(FLAGS.batch_size / 4):2 * int(FLAGS.batch_size / 4), :, :, :],
                                        LD_2: np.asarray(val_true_LD)[
                                              2 * int(FLAGS.batch_size / 4):3 * int(FLAGS.batch_size / 4), :, :, :],
                                        HD_2: np.asarray(val_true_HD)[
                                              2 * int(FLAGS.batch_size / 4):3 * int(FLAGS.batch_size / 4), :, :, :],
                                        LD_3: np.asarray(val_true_LD)[
                                              3 * int(FLAGS.batch_size / 4):4 * int(FLAGS.batch_size / 4), :, :, :],
                                        HD_3: np.asarray(val_true_HD)[
                                              3 * int(FLAGS.batch_size / 4):4 * int(FLAGS.batch_size / 4), :, :, :],
                                    })
                            ...
                            for j in range(int(math.ceil(len(l_val_files) / FLAGS.batch_size))):
                                ...
                                val_detect_loss_list.append(val_detect_losses_0)
