...
with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    input_x = SimpleITK.GetArrayFromImage(SimpleITK.ReadImage("../mydata/BRATS2015/testT1/13_96.tiff")).astype(
        'float32')
    input_l = SimpleITK.GetArrayFromImage(SimpleITK.ReadImage("../mydata/BRATS2015/testLabel/13_96.tiff")).astype(
        'float32')
    input_y = SimpleITK.GetArrayFromImage(SimpleITK.ReadImage("../mydata/BRATS2015/testT2/13_96.tiff")).astype(
        'float32')
    ...
    full_x = np.concatenate(
        [np.asarray(fx_)[0, :, :, 0:1] * 255, np.asarray(fx_)[0, :, :, 0:1] * 255, np.asarray(fx_)[0, :, :, 0:1] * 255],
        axis=-1)
    print(full_x.shape)
