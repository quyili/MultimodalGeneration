...
\begin{itemize}
	...
	We use randomly generated structural feature maps to fuse with randomly selected lesion labels and then synthesize the registered multimodal MRIs through the generator. We explore a variety of lesion generation guidence methods to achieve effective mapping of lesions based on input lesion labels during multimodal MRI synthesis. In training, no registration data is required, no additional data labels are required except for the lesion label, the synthetic data is registered, and the randomly selected lesion label is the lesion label of the synthetic data. Our approach enables fast and easy construction of registered multimodal MRI datasets with label.
	...
	We construct datasets with different proportions of synthetic and real data to train the lesion segmentation network, and verify that the synthetic data can be mixed with the real data in medical image intelligent processing tasks to improve the generalization ability of the model. Compared with the traditional subjective evaluation method of synthetic image quality, our method can verify the availability of synthetic data objectively and truthfully.
...
多模态MRI生成阶段我们产出一个条件生成器，其以结构特征图为输入，能根据不同的独热条件向量生成不同模态的MRI，并且可在结构特征图上添加病灶标签使得生成的MRI具有对应的病灶信息。该阶段我们训练的模型模块包括一个结构特征与病灶标签融合图编码器、四个MRI模态的病灶分割器、一个MRI编码器、一个MRI解码器、一个MRI鉴别器和一个MRI编码鉴别器。
...
在合成数据分割检测阶段，我们根据真实的数据为每个MRI模态单独训练一个病灶分割器，并在真实数据集中进行分割能力测试。此外，我们使用由不同比重的合成数据和真实数据构建的数据集来对病灶分割网络进行训练，训练充分后再在真实测试数据集上进行分割能力测试，对比各项测试结果，以验证合成数据在模型训练中的可用性。该阶段我们训练的模型包括每个模态的MRI的基于不同数据比重的病灶分割器。
...
在训练过程中，我们通过鉴别器$D_f$进行对抗训练以使解码器解码的结构特征图更逼真，此外，还通过特征鉴别器$FD_f$进行对抗训练，以此使得编码器$EC_f$能够将结构特征图f编码至标准正态分布。完整的损失函数如下，其中，$\omega_{i,j}$为各损失项的权重：
...
\begin{itemize}
	\item\textbf{结构特征对抗性损失Structural Feature Adversarial Loss} 
	这部分损失包括：使得结构特征图编码服从正态分布的对抗性损失，以及使得随机正态分布矩阵解码出结构特征图更逼真的对抗性损失
	This part of the loss includes: adversarial loss to constrain the structural feature map code obey normal distribution, and adversarial loss to constrain structural feature map decode by normal distribution matrix more realistic
	\begin{center}
	...
	\item \textbf{结构特征编码的监督损失Supervision Loss of Structural Features Code} 
	这项损失使得结构特征图中间编码结果服从正态分布，其中，$mean()$函数为均值函数。This loss item constrains the encode results of structural feature map obey normal distribution, where $mean()$ is a mean function.
	...
	\item \textbf{Mask生成器损失 Mask Generator Loss}
	从$f$生成对应$mask$的Mask生成器损失
	The mask generator loss of generate $mask$ from $f$
	\begin{center}
