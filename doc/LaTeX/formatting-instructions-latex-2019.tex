...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State $mask = resize\_with\_crop\_or\_pad(mask, shape[1], shape[2])$
	\end{algorithmic}  
\end{algorithm}
%\begin{quote}
...
\begin{algorithm}[t]
	...
	output result
	\begin{algorithmic}[1]
		\Function {GetMaskFromF}{$img$}
		\State $contours = OpenCV.findContours(img)$
		\State $img = OpenCV.drawContours(img，contours)$
		\State \Return img
		\EndFunction
		\State
		\State $mae=0.05$
...
如表~\ref{label_test}所示，我们在BRATS2015训练数据经过100个epoch的充分训练后，在真实的测试数据集上，分割测试结果达到了0.033的MSE和0.954的SSIM\cite{93wang2004image}。之后我们使用这个表现优秀的分割网络对我们的合成数据进行分割测试，在与真实测试数据集相同数据量的合成数据集上......
