...
\begin{itemize}
	...
	\begin{center}
		$loss_{D}=\sum\limits_{i=0}(\Vert{d_{i}-1}\Vert_{2}^{2}+\Vert{d_{g,i}}\Vert_{2}^{2}+\Vert{c_{i}-i}\Vert_{2}^{2}+\Vert{c_{g,i}-i}\Vert_{2}^{2})$
	\end{center}

	\item \textbf{Generation adversarial and category guidance loss}
	\begin{center}
		$loss_{G}=\sum\limits_{i=0}(\Vert{d_{g,i}-1}\Vert_{2}^{2}+\Vert{c_{g,i}-i}\Vert_{2}^{2})$
	\end{center}
	...
	\item \textbf{Semantic consistency loss}
	\begin{center}
		$loss_{trans,code}=\sum\limits_{i=0}(\Vert{code_g-code_{g,i}}\Vert_{2}^{2})+\sum\limits_{j=0,j\neq i}\sum\limits_{i=0}(\Vert{code_{g,i}-code_{g,j}}\Vert_{2}^{2})$
	\end{center}
...
\begin{itemize}
	...
	Each MRI modality  is segmented by a common complete segmentor.
	\item \textbf{Single lesion encoder + multiple lesion decoders} 
	Each MRI modality  is segmented by a segmentors which is combined by a common lesion encoder and different lesion decoders. 
	\item \textbf{Multiple segmentors} 
	Each MRI modality  is segmented by a different complete segmentor.
\end{itemize}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\columnwidth]{figures/segmentation}
	\caption{lesion segmentation.}
	\label{segmentation}
\end{figure}
Besides, we will also independently train the segmentor of these three schemes. In different experiments, we will train the segmentor in each scheme with different amounts of synthetic data or real data. The loss function of the training is as follows, where $label_i$ is the supervised label and $label_{r,i}$ is the segmentation result:
\begin{center}
	$loss_{l,independent}=\sum\limits_{i=0}\Vert{label_i-label_{r,i}}\Vert_{2}^{2}$
\end{center}
...
\subsection{Contrast experiments of lesion generation guidance method}
...
\subsection{Verification experiments of synthetic data availability}
...
\section{Results}
\subsection{Results of lesion generation guidance method contrast experiments}
\begin{table}[t]
	\caption{Results of lesion generation guidance method contrast experiments}\smallskip
	\centering
...
As shown in the Table~\ref{label_test}, the segmentation test results reach the MSE of 0.026 and the Dice Score of 0.915 of the segmentors trained on the real data. And these segmentators segment the unfiltered synthetic data from the Multiple segmentors scheme to get MSE 0.043 and Dice Score 0.838. From results, three different lesion label generation methods have achieved good segmentation results, where the Multiple segmentors scheme is best. 

\subsection{Results of synthetic data availability verification experiments}
\begin{table}[t]
	\caption{Results of synthetic data availability verification experiments}\smallskip
	\centering
	\resizebox{.95\columnwidth}{!}{
		\smallskip\begin{tabular}{lllllll}
			...
			2 & 0.5Folds & 0  		&0 			&- &0.032 &0.902 \\
			3 & 0 	 	 & 1Fold  	&0 			&- &0.205 &0.708 \\
			4 & 0 	 	 & 2Folds  	&0 			&random mixing &0.206 &0.736 \\
			5 & 0 		 & 3Folds  	&0 			&random mixing &0.205 &0.754 \\
			6 & 0.1Folds & 1Fold  	&0 			&synthetic first &0.031 &0.908 \\
			7 & 0.1Folds & 2Folds   &0 			&synthetic first &0.028 &0.907 \\
			8 & 0.1Folds & 3Folds   &0 			&synthetic first &0.030 &0.907 \\	
			9 & 0.2Folds & 0.8Folds &0  		&random mixing &0.041 &0.850 \\
			10& 0.5Folds & 0.5Folds &0  		&random mixing &0.031 &0.904 \\
			11& 0.8Folds & 0.2Folds &0  		&random mixing &0.024 &0.935 \\
			...
			18& 1Fold 	 &0 		&  0.2Folds &random mixing &0.027 &0.911 \\
			19& 1Fold 	 &0 		&  0.5Folds &random mixing &0.025 &0.927 \\
			20& 1Fold    &0 		&  0.8Folds &random mixing &0.026 &0.920 \\
...
As shown in Table~\ref{use_test}，the results of NO.3-NO.5 show that the synthetic data can not completely replace the real data in training.            
...
As Figure~\ref{generated_f} shows the examples of the structural feature map we generated from the random normal distribution matrix. Figure~\ref{generated_mri} shows the examples of generated structural feature maps, generated masks, selected lesion segmentation labels, and generated multi-modal MRI.
\begin{figure}
	\centering
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{figures/F_to_MRI}
