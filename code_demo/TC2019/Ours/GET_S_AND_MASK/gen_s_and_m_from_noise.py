...
def train():
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        while index <= FLAGS.num:
            ...
            SimpleITK.WriteImage(SimpleITK.GetImageFromArray(np.asarray(s)[0, :, :, :]),
                                 "./test_images/S/" + str(index) + ".mha")
            SimpleITK.WriteImage(SimpleITK.GetImageFromArray(np.asarray(m)[0, :, :, :]),
                                 "./test_images/M/" + str(index) + ".mha")
